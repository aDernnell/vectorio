...
import * as mat2 from '../src/mat2/namespace';
import * as mat3 from '../src/mat3/namespace';
import * as mat4 from '../src/mat4/namespace';
import * as vec2 from '../src/vec2/namespace';
import * as vec3 from '../src/vec3/namespace';
...
describe('transform consistency', function () {
    ...
    it('checks mat4 rotation', function () {
        ...
        expect(m4A).toEqualMat(m4B);
        expect(m4B).toEqualMat(m4C);
    ...
    it('checks TRS decomposition', function () {
        ...
        expect(mat2.extractScaling(vec2.create(), m2)).toEqualVec(vec2.of(2, 3));

        const m3 = mat3.create();
        mat3.fillTranslation(m3, vec2.of(2, 3));
        mat3.rotate(m3, m3, 0.1);
        mat3.scale(m3, m3, vec2.of(2, 3));
        expect(mat3.extractTranslation(vec2.create(), m3)).toEqualVec(vec2.of(2, 3));
        expect(mat3.extractAngle(m3)).toBeCloseTo(0.1, 6);
        expect(mat3.extractScaling(vec2.create(), m3)).toEqualVec(vec2.of(2, 3));

        const m4 = mat4.create();
        mat4.fillTranslation(m4, vec3.of(2, 3, 4));
        mat4.rotateZYX(m4, m4, vec3.of(0.1, 0.2, 0.3));
        mat4.scale(m4, m4, vec3.of(2, 3, 4));
        expect(mat4.extractTranslation(vec3.create(), m4)).toEqualVec(vec3.of(2, 3, 4));
        expect(mat4.extractEulerAngles(vec3.create(), m4)).toEqualVec(vec3.of(0.1, 0.2, 0.3));
        expect(mat4.extractScaling(vec3.create(), m4)).toEqualVec(vec3.of(2, 3, 4));
    });
