...
import { quat, QUAT_IDENTITY, quatEquals } from '../../quat/quat-core';
import { quatFillEuler, quatFillRotation, quatFillRotationTo, quatRotateX, quatRotateY, quatRotateZ } from '../../quat/quat-rot';
import { vec3, VEC3_X, VEC3_Y, VEC3_Z, vec3Equals, vec3Normalize, vec3QuatTransform } from '../../vec3';
...
describe('quat-rot', () => {
    ...
    describe('rotateX', () => {
        ...
        it('leaves the quaternion unchanged when rotating by 0', () => {
            ...
            expect(quatEquals(out, source)).toBe(true);
        });
        ...
        it('output a modified parameter', () => {
            ...
            expect(out).toBe(q);
            expect(quatEquals(q, quatFillRotation(quat(), VEC3_X, Math.PI / 2))).toBe(true);
        });
    });
    ...
    describe('rotateY', () => {
        ...
        it('output a modified parameter', () => {
            ...
            expect(out).toBe(q);
            expect(quatEquals(q, quatFillRotation(quat(), VEC3_Y, Math.PI / 2))).toBe(true);
        });
    });
    ...
    describe('rotateZ', () => {
        ...
        it('output a modified parameter', () => {
            ...
            expect(out).toBe(q);
            expect(quatEquals(q, quatFillRotation(quat(), VEC3_Z, Math.PI / 2))).toBe(true);
        });
    });
    ...
    describe('fillEuler', () => {
        ...
        it('matches individual axis rotations for single angles', () => {
            ...
            expect(quatEquals(out, quatFillRotation(quat(), VEC3_X, angle))).toBe(true);

            quatFillEuler(out, 0, angle, 0);
            expect(quatEquals(out, quatFillRotation(quat(), VEC3_Y, angle))).toBe(true);

            quatFillEuler(out, 0, 0, angle);
            expect(quatEquals(out, quatFillRotation(quat(), VEC3_Z, angle))).toBe(true);
        });
    ...
    describe('fillRotationTo', () => {
        ...
        it('rotates from X to Y axis', () => {
            ...
            expect(vec3Equals(v, VEC3_Y)).toBe(true);
        });
        ...
        it('rotates from Y to Z axis', () => {
            ...
            expect(vec3Equals(v, VEC3_Z)).toBe(true);
        });
        ...
        it('rotates from Z to X axis', () => {
            ...
            expect(vec3Equals(v, VEC3_X)).toBe(true);
        });
        ...
        it('transforms arbitrary normalized vectors to their target', () => {
            ...
            expect(vec3Equals(v, b)).toBe(true);
